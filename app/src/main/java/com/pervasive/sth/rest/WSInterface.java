...
public class WSInterface {
	...
	private final RESTClient _deviceClient;
	private final RESTClient _deleteClient;
	private final RESTClient _foundClient;
	private final RESTClient _audioClient;
	private final RESTClient _pictureClient;


	public WSInterface() {
		_deviceClient = new RESTClient(BASE_URI + DEV_PATH);
		_deleteClient = new RESTClient(BASE_URI + DEL_PATH);
		_foundClient = new RESTClient(BASE_URI + FOUND_PATH);
		_audioClient = new RESTClient(BASE_URI + AUDIO_PATH);
		_pictureClient = new RESTClient(BASE_URI + PIC_PATH);
		_deviceClient.addHeader("content-type", "application/json");
		_deleteClient.addHeader("content-type", "text/plain");
		_foundClient.addHeader("content-type", "text/plain");
		_audioClient.addHeader("content-type", "application/json");
		_pictureClient.addHeader("content-type", "application/json");
	}
	...
	public void updateDeviceEntry(Device device) throws Exception {
		...
		_deviceClient.executePost(jsonDevice.toString());
	...
	public Device retrieveDevice() throws Exception {
		...
		JSONObject jsonDevice = new JSONObject(_deviceClient.executeGet());
	...
	public void updateTreasureStatus(Boolean status) throws Exception {
		...
		_foundClient.executePost(status.toString());
	}

	public boolean retrieveTreasureStatus() throws Exception {
		return Boolean.parseBoolean(_foundClient.executeGet());
	}

	public void deleteDevice(String id) throws Exception {
		_deleteClient.executePost(id);
	}
	...
	public void uploadAudio(Media mediaFile) throws Exception {
		...
		_audioClient.executePost(jsonAudio.toString());

		Log.d("WSInterface", "updated audio info.");
	}

	public Media retrieveAudio() throws Exception {
		JSONObject jsonAudio = new JSONObject(_audioClient.executeGet());
	...
	public void uploadPicture(Media mediaFile) throws Exception {
		...
		_pictureClient.executePost(jsonPicture.toString());

		Log.d("WSInterface", "updated picture info.");
	}

	public Media retrievePicture() throws Exception {
		JSONObject jsonPicture = new JSONObject(_pictureClient.executeGet());
